const cashRetirementTable = document.querySelector(".module-table");
...
cashRetirementTable.innerHTML = rows.length > 0 ? (
    `<table>
        <thead>
            <tr>
    ...
    `<div class="call-to-action">
        ...
        <div>
            ...
            <p>When Ofofon logs his cash retirement, they will show up here</p>
        </div>
        <div class="cta-box">
            <button onclick="handleOpenCashRetirementModal(event)">
                <span>Add New Cash Retirement</span>
            </button>
        </div>
    </div>`
);

cashRetirementForm.innerHTML = (`
    <form>
        ...
        <div class="row form-cta">
            <button type="reset" onclick="handleCloseCashRetirementModal()">
                <span>Cancel</span>
            </button>
...
function handleCloseCashRetirementModal() {
    cashRetirementModal.classList.add("close-modal");
    document.body.style.overflow = "auto";
    markedForDeHighlighting.forEach((item) => {
        item.style.opacity = 1;
        item.style.pointerEvents = "auto";
...
function handleOpenConfirmationModal(e) {
    ...
    cashRetirementConfirmationModal.classList.remove("close-modal");
    document.body.style.overflow = "hidden";
    markedForDeHighlighting.forEach((item) => {
        item.style.opacity = 0.1;
        item.style.pointerEvents = "none";
...
function handleCloseConfirmationModal() {
    cashRetirementConfirmationModal.classList.add("close-modal");
    document.body.style.overflow = "auto";
    markedForDeHighlighting.forEach((item) => {
        item.style.opacity = 1;
        item.style.pointerEvents = "auto";
