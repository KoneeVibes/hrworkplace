const timeSheetTable = document.querySelector(".module-table");
const timeSheetModal = document.querySelector(".time-sheet-module-modal");
const timeSheetForm = document.querySelector(".module-modal-form");
const timeSheetConfirmationModal = document.querySelector(".time-sheet-confirmation-modal");
...
timeSheetTable.innerHTML = rows.length > 0 ? (
    ...
    `<div class="call-to-action">
        ...
        <div class="cta-box">
            <button onclick="handleOpenTimeSheetModal(event)">
...
timeSheetForm.innerHTML = (`
    <form>
        ...
        <div class="row form-cta">
            <button type="reset" onclick="handleCloseTimeSheetModal()">
                <span>Cancel</span>
            </button>
...
function handleOpenTimeSheetModal(e) {
    e.stopPropagation();
    timeSheetModal.classList.remove("close-modal");
    document.body.style.overflow = "hidden";
    markedForDeHighlighting.forEach((item) => {
        item.style.opacity = 0.1;
        item.style.pointerEvents = "none";
...
function handleCloseTimeSheetModal() {
    timeSheetModal.classList.add("close-modal");
    document.body.style.overflow = "auto";
    markedForDeHighlighting.forEach((item) => {
        item.style.opacity = 1;
        item.style.pointerEvents = "auto";
...
function handleOpenConfirmationModal(e) {
    ...
    timeSheetConfirmationModal.classList.remove("close-modal");
    document.body.style.overflow = "hidden";
    markedForDeHighlighting.forEach((item) => {
        item.style.opacity = 0.1;
        item.style.pointerEvents = "none";
...
function handleCloseConfirmationModal() {
    timeSheetConfirmationModal.classList.add("close-modal");
    document.body.style.overflow = "auto";
    markedForDeHighlighting.forEach((item) => {
        item.style.opacity = 1;
        item.style.pointerEvents = "auto";
...
window.addEventListener("click", (e) => {
    ...
    if (!timeSheetModal.classList.contains("close-modal") && !timeSheetModal.contains(e.target)) {
        handleCloseTimeSheetModal();
    }
